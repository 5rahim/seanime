...
func (m *Anime) getAnimeEntry(call goja.FunctionCall) goja.Value {
	...
	if mediaId == 0 {
		_ = reject(m.vm.ToValue(m.vm.NewGoError(errors.New("anilist platform not found"))))
		return m.vm.ToValue(promise)
	}

	database, ok := m.ctx.database.Get()
	if !ok {
		_ = reject(m.vm.ToValue(m.vm.NewGoError(errors.New("database not found"))))
		return m.vm.ToValue(promise)
	}
	...
	if !ok {
		_ = reject(m.vm.ToValue(m.vm.NewGoError(errors.New("metadata provider not found"))))
		return m.vm.ToValue(promise)
	}

	fillerManager, ok := m.ctx.fillerManager.Get()
	if !ok {
		_ = reject(m.vm.ToValue(m.vm.NewGoError(errors.New("filler manager not found"))))
		return m.vm.ToValue(promise)
	}
	...
	go func() {
		...
		if animeCollection == nil {
			_ = reject(m.vm.ToValue(m.vm.NewGoError(errors.New("anilist collection not found"))))
			return
		}
		...
		if err != nil {
			_ = reject(m.vm.ToValue(m.vm.NewGoError(err)))
			return
		}
