...
// BindFilesystem binds the filesystem module to the Goja runtime.
// Permissions needed: filesystem
func (a *AppContextImpl) BindFilesystem(vm *goja.Runtime, logger *zerolog.Logger, ext *extension.Extension) {
	filesystemObj := vm.NewObject()

	filesystemObj.Set("fileFromPath", NewFileFromPath)
	filesystemObj.Set("fileFromBytes", NewFileFromBytes)
	filesystemObj.Set("fileFromMultipart", NewFileFromMultipart)
	filesystemObj.Set("fileFromURL", func(url string, secTimeout int) (*File, error) {
		if secTimeout == 0 {
			secTimeout = 120
		}
		...
		return NewFileFromURL(ctx, url)
	})

	_ = vm.Set("$filesystem", filesystemObj)
}
...
func normalizeName(fr FileReader, name string) string {
	...
	// name
	// ---
	cleanName := util.Snakecase(strings.TrimSuffix(name, originalExt))
	if length := len(cleanName); length < 3 {
		// the name is too short so we concatenate an additional random part
		cleanName += util.RandomStringWithAlphabet(10, randomAlphabet)
	} else if length > 100 {
	...
	return fmt.Sprintf(
		"%s_%s%s",
		cleanName,
		util.RandomStringWithAlphabet(10, randomAlphabet), // ensure that there is always a random part
		cleanExt,
	)
}
