...
func NewMediaInfoExtractor(path string, route string) (*MediaInfoExtractor, error) {
...
func (e *MediaInfoExtractor) GetInfo() (mi *MediaInfo, err error) {
	...
	go func() {
		savePath := fmt.Sprintf("%s/%s/info.json", Settings.Metadata, e.sha)
		if err := getSavedInfo(savePath, mi); err == nil {
			log.Printf("Using mediainfo cache on filesystem for %s", e.path)
			close(readyChan)
			return
		}
