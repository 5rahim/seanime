...
import (
	...
	"github.com/seanime-app/seanime/internal/videofile"
	"io"
	"os"
	"path/filepath"
)
...
func GetInfo(path string, sha string, route string) (*videofile.MediaInfo, error) {
	defer printExecTime("mediainfo for %s", path)()

	me, err := videofile.NewMediaInfoExtractor(path, route)
	if err != nil {
		return nil, err
	}
