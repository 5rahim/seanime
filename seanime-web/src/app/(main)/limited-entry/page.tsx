...
import { useMediaDetails, useSimpleMediaEntry } from "@/app/(main)/entry/_lib/media-entry"
...
export default function Page() {
    ...
    const { mediaEntry, mediaEntryLoading } = useSimpleMediaEntry(mediaId)
    const { mediaDetails, mediaDetailsLoading } = useMediaDetails(mediaId)

    useEffect(() => {
        if (!mediaId || (!mediaEntryLoading && !mediaEntry)) {
            router.push("/")
        }
