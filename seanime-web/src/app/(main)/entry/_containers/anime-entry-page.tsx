...
export const __anime_debridStreamingViewActiveAtom = atom(false)
...
export function AnimeEntryPage() {
    ...
    const [isDebridStreamingView, setIsDebridStreamingView] = useAtom(__anime_debridStreamingViewActiveAtom)
    ...
    React.useLayoutEffect(() => {
        if (!animeEntryLoading &&
            ...
            (
                ...
                (serverStatus?.debridSettings?.enabled && serverStatus?.debridSettings?.fallbackToDebridStreamingView)
            ) &&
            !switchedView.current
        ) {
            switchedView.current = true
            if (serverStatus?.debridSettings?.enabled && serverStatus?.debridSettings?.fallbackToDebridStreamingView) {
                setIsDebridStreamingView(true)
            } else if (serverStatus?.torrentstreamSettings?.enabled && serverStatus?.torrentstreamSettings?.fallbackToTorrentStreamingView) {
    ...
    useUnmount(() => {
        ...
        setIsDebridStreamingView(false)
    })
            ...
            <div className="px-4 md:px-8 relative z-[8]">
                ...
                >
                    <AnimatePresence mode="wait" initial={false}>
                        {(!isTorrentStreamingView && !isDebridStreamingView) && <PageWrapper
                            key="episode-list"
                            className="relative 2xl:order-first pb-10"
                            {...{
                                initial: { opacity: 0, y: 60 },
                        ...
                        {isDebridStreamingView && <PageWrapper
                            key="torrent-streaming-episodes"
                            className="relative 2xl:order-first pb-10 lg:pt-0"
                            {...{
                                initial: { opacity: 0, y: 60 },
